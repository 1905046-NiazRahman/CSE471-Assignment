...
- In the 3rd point of methodology, it is mentioned that the researchers evaluated “Forecasting”, “Classification” and  “Reasoning” in the paper. But actually, the forecasting is only evaluated, and the other two are not evaluated, which is stated as a limitation in Appendix A of the paper.
  
- There are three methods used in the paper that use LLMs for time series forecasting: OneFitsAll, Time-LLM, and CALF. The names are present in the 3rd figure, also these are some major experimental topics, but any mention of these is not present in the blog.
