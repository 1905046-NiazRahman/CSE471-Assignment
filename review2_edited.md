...
5. The blog post mentions that the researchers conducted ablation studies, but it does not provide specifics about the types of ablations performed. The paper describes three specific ablation methods: removing the LLM component, replacing it with an attention layer, and replacing it with a transformer block. The figures in the key contribution section state these terms, but there are no mentions about these ablation methods in the blog. Including these details in the blog post would significantly enhance its informativeness.
